...
![Ruff](https://github.com/MySelfZalin/ican-scraper/actions/workflows/ruff.yml/badge.svg)
...
Данные с датчика непрерывного мониторинга глюкозы существуют в закрытой экосистеме. Официальные приложения привязаны к телефону, нет публичного API - и другого способа вытащить данные тоже. А если данные нужны? Возможно своя аналитика или интеграция в свои скрипты?
...
- **Отказоустойчивость** Если с сайтом что-то произошло (протухла сессия или ответил с таймаутом), то парсер распознает это и после 3 неудачных попыток залогинится заново. Благодаря сохранённому профилю браузера `data/browser_profile` повторные логины случаются редко и сессия не сбрасывается при каждом перезапуске

- **Аккуратное завершение** Docker контейнер корректно обрабатывает остановку: браузер закрывается и мусорных файлов не остаётся
...
git clone https://github.com/MySelfZalin/ican-scraper.git
cd ican-scraper
